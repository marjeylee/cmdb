(function($) {
	$.learuntab = {
		...
		refreshTab: function() {
			const currentId = $('.page-tabs-content').find('.active').attr('data-id');
			const target = $('.LRADMS_iframe[data-id="' + currentId + '"]');
		...
		addTab: function() {
			...
			if (flag) {
				let str = '<a href="javascript:;" class="active menuTab" data-id="' + dataUrl + '">' + menuName +
					' <i class="fa fa-remove"></i></a>';
				$('.menuTab').removeClass('active');
				let str1 = '<iframe class="LRADMS_iframe" id="iframe' + dataId + '" name="iframe' + dataId +
		...
		loadMenu: function() {
			let data = [{
				...
				},
				{
					...
					"F_UrlAddress": "./html/table_operate.html",
					"F_Target": "iframe",
					"F_IsMenu": 1,
					"F_AllowExpand": 1,
					"F_IsPublic": 0,
						...
						$.each(childNodes, function(i) {
							...
							if (subchildNodes.length > 0) {
								...
								$.each(subchildNodes, function(i) {
									...
									_html += '<li><a class="menuItem" data-id="' + subrow.F_ModuleId + '" href="' + subrow.F_UrlAddress +
										'"><i class="' + subchildNodesrow.F_Icon + '"></i>' + subchildNodesrow.F_FullName + '</a></li>';
								});
								_html += '</ul>';

							} else {
								_html += '<a class="menuItem" data-id="' + subrow.F_ModuleId + '" href="' + subrow.F_UrlAddress +
									'"><i class="' + subrow.F_Icon + '"></i>' + subrow.F_FullName + '</a>';
							}
							_html += '</li>';
						});
						_html += '</ul>';
